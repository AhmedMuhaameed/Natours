...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      //2)check if user still exist (may be deleted)
      const currentUser = await User.findById(decoded.id);
      if (!currentUser) return next();

      //3)check if user changes password after token was issued
      if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  //there's fields required by DB we must stop validation now
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  //2)if token not expired and there's a user ,set new password
  if (!user) {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  //2)Check if the posted current password is correct one
